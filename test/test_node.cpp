...
TEST_P(NodeTest, LeaderFailWithArbiter) {
    ...
    // Enable following line cause old leader's log is up-to-data before clean and restart. 
    // If no more new log is applied , new leader believe no new log should be send to old
    // leader and the old leader will never catchup, maybe a BUG?
    // 
...
TEST_P(NodeTest, LeaderFollowerFailWithArbiter) {
    std::vector<braft::PeerId> peers;
    for (int i = 0; i < 3; i++) {
        braft::PeerId peer;
        peer.addr.ip = butil::my_ip();
    ...
    LOG(WARNING) << "stop leader" << leader_addr;

    // 5. restart follower
    ASSERT_EQ(0, cluster.start(follower_addr));
    LOG(WARNING) << "restart follower " << follower_addr;

    // the follower can't become new leader, it don't has all committed log
    sleep(5);
    ASSERT_EQ(cluster.leader(), nullptr);

    // 6. restart leader
    cluster.start(leader_addr);
    LOG(WARNING) << "restart leader" << leader_addr;

    cluster.wait_leader();
    // the old leader must become new leader
    ASSERT_EQ(cluster.leader()->node_id().peer_id.addr, leader_addr);
